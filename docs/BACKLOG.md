...
### `LobbyView` "Random Question" and "Random League Question" share slug `football` — per-row loading indicator can't distinguish them
- **Severity**: Low — surfaces after the 2026-06-14 lobby pending-state work
- **What**: Both Football "Random" rows call `onStartGame("football", ..., {scope: ...})` with different filter scopes (`random_any` vs `random_league_level`). The shared `starting` state at `LobbyView` is keyed on slug, so when one is in flight `starting === "football"` is true for both — only the first row currently shows the per-row "Starting…" label as a workaround. Disable-all behaviour is unaffected; this is purely the per-row visual cue.
- **Fix**: Key the starting state on a composite identifier that includes the scope (e.g. `football:random_any` vs `football:random_league_level`), or refactor to give each row a unique slug at the data-model level.
- **Files**: `frontend-react/src/components/game/lobby/LobbyView.tsx` (`FootballScreen` — the two "Random" `NavRow`s)
...
### Daily challenge: backend safeguard against post-completion replay
- **What**: `DailyChallengeService.startDailyChallenge` only calls `gameService.abandonActiveGamesForPlayer(playerId)` before creating a new game — it does NOT check whether the player already has a `COMPLETED` daily for this category today. A player who has already completed today's Football daily can start it again unrestricted. Current behaviour is consistent with the explicitly trust-based design (no `daily_challenge_results` table), but if the UI ever surfaces "you've already played this" or "see your result" affordances, the backend needs a matching gate.
- **Why deferred**: Trust-based model is intentional for now; no exploit, no rage-quit risk. Becomes worth doing once played-state tracking lands client-side (see practice-set item "UX: No indication of which daily challenges you've already played"), so the UI and backend agree on what "consumed" means.
- **When to pick up**: When implementing played-state UI, or when introducing any leaderboard / shared-result feature where double-attempts would distort comparisons.
- **Files**: `DailyChallengeService.java:90` (`startDailyChallenge`), `DailyChallengeRepository.java` (would need a `findCompletedDailyForCategoryAndDate(playerId, categoryId, date)` query), `games` table (already has `status` column).
...
| UX: `/daily` error state offers no retry | 2026-06-13 | Destructured `refresh` from `useDailyChallenge`, added a Refresh button to the `/daily` error branch that's disabled while loading. |
| BUG: `useGameLoop.startNewGame` sends to wrong API path + win-overlay reshape | 2026-06-13 | UX reframe: removed the broken "Play Again" affordance from the Daily Challenge win overlay entirely (one-attempt-per-day; Play Again is misleading). Daily win now shows Share + Back to Lobby only; post-win Exit skips the mid-game progress-loss confirm. Renamed `debugGameType` → `gameType` prop on `MatchView` and conditioned the Play Again button on `gameType !== "daily-challenge"`. Defensive fixes to `startNewGame` for the underlying stale-closure bug: hardcoded `/api/freeplay/start` and passed `"freeplay"` literal to `saveGameState`. |
| UX: Winning, then exiting, warns "your progress will be lost" | 2026-06-13 | Closed as a side-effect of the win-overlay reshape above — post-win "Exit to lobby" now calls `onExit()` directly; mid-game header Exit still goes through the confirm dialog. |
| UX: Enter-key collision can throw the wrong dart | 2026-06-13 | One-line fix in `EntitySearch.handleKeyDown`: `e.stopPropagation()` on Enter when the input has text, so the keydown doesn't bubble to `MatchView`'s wrapper handler and throw the previously-staged dart. Empty-input fast path (Enter throws the staged dart) preserved. Also closes the sub-4-char hole where typing a partial name + Enter would throw the OLD staged answer. |
| UX: No pending state when starting a game from the lobby — double-click fires duplicate starts | 2026-06-14 | Single shared `starting: string \| null` state at `LobbyView` level (slug of the in-flight row, or null when idle). Threaded down through `NavScreenRenderer` to every screen; each `NavRow` gets new `disabled` + `loading` props. Wrapped `startGame` and added `startDailyChallenge` wrapper with re-entry guard + set-before / clear-in-finally. Daily card shows "STARTING…" on the clicked card; all other rows visibly disabled. `onStartGame`/`onStartDailyChallenge` prop types relaxed to `Promise<void> \| void` so the wrapper can await. |
